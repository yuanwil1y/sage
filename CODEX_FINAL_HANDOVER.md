...
This document is the handoff after the post-PR-21 deep review and the follow-up implementation work. Do **not** redo the earlier Python streaming architecture. Start from the open draft PRs below and validate/merge them deliberately.
...
Still validate on Windows:
1. Build `gamebar-widget/Service/SageWidgetService.csproj` x64 Release.
2. Build the Game Bar widget/UWP project.
3. Start widget + service, accumulate a nontrivial old cursor, restart only SageWidgetService, publish events before the widget detects the new session, and verify the first retained events are shown immediately.
4. Feed interleaved voice/chat subtitles with one source overflowing by 2+ and verify the other source is never pruned incorrectly.

A temporary compile workflow may appear in branch history; remove/ignore validation-only workflow changes and preserve the final business diff.
...
- CI now runs display/ROI/screen-capture/Named-Pipe tests.
- Added Windows artifact job to build native helper, build SageWidgetService, build an unsigned Game Bar MSIX/AppX via `gamebar-widget/build.ps1`, then run artifact-dependent local-service integration tests.
...
If two non-primary monitors have the same resolution/DPI and DXcam's public enumeration cannot uniquely map a new Qt selection, current code intentionally rejects the selection instead of guessing. If real Windows testing shows a better stable identity is available (Win32 monitor handle / DXGI output identity), improve the mapping rather than restoring index assumptions.
...
- Windows Python/supervision job including `test_pipe_server.py`: green.
...
1. Validate PR #28 and merge it.
2. Merge PR #29 after its already-green Python checks are still current.
3. Rebase/update PR #31 onto the new `main`, preserving `test_startup_options.py`; run Windows pipe fault injection; merge #31.
4. Rebase/update PR #30 **last**, preserving the union of all tests/workflow changes, then run all three jobs and physical monitor/Game Bar acceptance before merging.
...
## Remaining issue #15 acceptance beyond CI
...
Do not merge a PR just because it compiles; run the relevant Windows acceptance described in the handover.
```
